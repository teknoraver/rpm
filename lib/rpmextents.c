...
int extentsVerifySigs(FD_t fd, int print_content)
{
	...
	if (Fseek(fd, footer.offsets.checksig_offset, SEEK_SET) < 0) {
		rpmlog(RPMLOG_ERR,
		       _("extentsVerifySigs: Failed to seek signature verification offset\n"));
		rc = -1;
		goto exit;
	}
	len = sizeof(rc);
	if (Fread(&rc, len, 1, fd) != len) {
		rpmlog(RPMLOG_ERR,
		       _("extentsVerifySigs: Failed to read Signature Verification RC\n"));
		rc = -1;
		goto exit;
	}
	...
	if (print_content) {
		...
		if (Fread(&content_len, len, 1, fd) != len) {
			rpmlog(RPMLOG_ERR,
			       _("extentsVerifySigs: Failed to read signature content length\n"));
			goto exit;
		}
		...
		if (content == NULL) {
			rpmlog(RPMLOG_ERR,
			       _("extentsVerifySigs: Failed to allocate memory to read signature content\n"));
			goto exit;
		}
		content[content_len] = 0;
		if (Fread(content, content_len, 1, fd) != content_len) {
			rpmlog(RPMLOG_ERR,
			       _("extentsVerifySigs: Failed to read signature content\n"));
			goto exit;
		}
 ...
 exit:
	...
	if (Fseek(fd, current, SEEK_SET) < 0)
		rpmlog(RPMLOG_ERR,
		       _("extentsVerifySigs: unable to seek back to original location\n"));
...
rpmRC extentsFooterFromFD(FD_t fd, struct extents_footer_t *footer)
{
	...
	if (Fread(footer, len, 1, fd) != len) {
		rpmlog(RPMLOG_ERR,
		       _("isTranscodedRpm: unable to read footer\n"));
		rc = RPMRC_FAIL;
		goto exit;
	}
 ...
 exit:
	if (Fseek(fd, current, SEEK_SET) < 0) {
		rpmlog(RPMLOG_ERR,
		       _("isTranscodedRpm: unable to seek back to original location\n"));
		rc = RPMRC_FAIL;
	}
	return rc;
}
