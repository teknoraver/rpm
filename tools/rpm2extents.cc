...
static int FDWriteDigests(FD_t fdi,
			  FD_t fdo, uint8_t algos[], uint32_t algos_len)
{
	...
	for (algo = 0; algo < algos_len; algo++) {
		fdFiniDigest(fdi, algos[algo], (void **)&filedigest,
			     &filedigest_len, 0);
		...
		if (Fwrite(filedigest, algo_digest_len, 1, fdo) !=
		    algo_digest_len) {
			rpmlog(RPMLOG_ERR,
...
static rpmRC FDWriteSignaturesValidation(FD_t fdo, int rpmvsrc, char *msg)
{
	...
	if (Fwrite(msg, content_len, 1, fdo) != content_len) {
		rpmlog(RPMLOG_ERR,
		       _
		       ("Unable to write signature verification output %s: %d, %s\n"),
...
static rpmRC validator(FD_t fdi, FD_t digesto, FD_t sigo,
		       uint8_t algos[], uint32_t algos_len)
{
	int rpmvsrc;
	rpmRC rc = RPMRC_FAIL;
	char *msg = NULL;
...
static rpmRC process_package(FD_t fdi, FD_t digestori, FD_t validationi)
{
	...
	rpmfi fi = NULL;
	char *msg = NULL;
	std::vector <struct digestoffset> offsets;
	uint32_t diglen;
	...
	if (isInDenyList(l.name)) {
		...
		while (next >= 0) {
			...
			const unsigned char *digestb =
			    rpmfiFDigest(fi, NULL, NULL);
			std::vector < unsigned char >digest(digestb,
							    digestb + diglen);
		...
		std::sort(offsets.begin(), offsets.end(), [diglen](const struct digestoffset &a, const struct digestoffset &b) {
			return memcmp(a.digest, b.digest, diglen) < 0;
		});
		...
		if (Fwrite(&offset_ix, sizeof(offset_ix), 1, fdo) != sizeof(offset_ix)) {
			rpmlog(RPMLOG_ERR,
			       _("Unable to write length of table\n"));
			rc = RPMRC_FAIL;
			goto exit;
		}
		len = sizeof(diglen);
		if (Fwrite(&diglen, len, 1, fdo) != len) {
			rpmlog(RPMLOG_ERR,
			       _("Unable to write length of digest\n"));
			rc = RPMRC_FAIL;
			goto exit;
		}
		...
		struct extents_footer_t footer = {.offsets =
			    {(off64_t) validation_pos,
			     ...
			     (off64_t) digest_pos},.magic = EXTENTS_MAGIC
		};
		len = sizeof(footer);
...
static off_t ufdTee(FD_t sfd, FD_t *fds, int len)
{
	...
	while (1) {
		...
		if (rdbytes > 0) {
			for (int i = 0; i < len; i++) {
				wrbytes =
				    Fwrite(buf, sizeof(buf[0]), rdbytes,
					   fds[i]);
				if (wrbytes != rdbytes) {
					rpmlog(RPMLOG_ERR,
