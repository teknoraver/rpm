...
struct digestoffset {
	...
	bool operator<(const digestoffset &other) const {
		return digest < other.digest;
	}
};

/* Rounds up pos to the next multiple of unit. Only works if unit is a power of two. */
...
static int FDWriteDigests(FD_t fdi, FD_t fdo, uint8_t algos[], uint32_t algos_len)
{
	const char *filedigest, *algo_name;
	size_t filedigest_len;
	...
	if (Fwrite(&fdilength, sizeof(fdilength), 1, fdo) != sizeof(fdilength)) {
		rpmlog(RPMLOG_ERR,
		       _("Unable to write input length %zd: %d, %s\n"),
		       fdilength, errno, strerror(errno));
		goto exit;
	}
	if (Fwrite(&algos_len, sizeof(algos_len), 1, fdo) != sizeof(algos_len)) {
		algo_digest_len = (uint32_t) filedigest_len;
		rpmlog(RPMLOG_ERR,
		       _("Unable to write number of digests: %d, %s\n"), errno,
		       strerror(errno));
		goto exit;
	}
	for (algo = 0; algo < algos_len; algo++) {
		...
		if (Fwrite(&algo_name_len, sizeof(algo_name_len), 1, fdo) != sizeof(algo_name_len)) {
			rpmlog(RPMLOG_ERR,
			       _("Unable to write digest algo name length: %d, %s\n"),
			       errno, strerror(errno));
			goto exit;
		}
		if (Fwrite(&algo_digest_len, sizeof(algo_digest_len), 1, fdo) != sizeof(algo_digest_len)) {
			rpmlog(RPMLOG_ERR,
		...
		if (Fwrite(algo_name, algo_name_len, 1, fdo) != algo_name_len) {
			rpmlog(RPMLOG_ERR,
			       _("Unable to write digest algo name: %d, %s\n"),
			       errno, strerror(errno));
			goto exit;
		}
		if (Fwrite(filedigest, algo_digest_len, 1, fdo) != algo_digest_len) {
			rpmlog(RPMLOG_ERR,
			       ...
			       algo_digest_len, filedigest_len, errno,
			       strerror(errno));
			goto exit;
		}
	}
...
static rpmRC FDWriteSignaturesValidation(FD_t fdo, int rpmvsrc, char *msg)
{
	...
	if (Fwrite(&rpmvsrc, sizeof(rpmvsrc), 1, fdo) != sizeof(rpmvsrc)) {
		rpmlog(RPMLOG_ERR,
	...
	if (Fwrite(&content_len, sizeof(content_len), 1, fdo) != sizeof(content_len)) {
		rpmlog(RPMLOG_ERR,
...
static rpmRC validator(FD_t fdi, FD_t digesto, FD_t sigo, uint8_t algos[], uint32_t algos_len)
{
	int rpmvsrc;
	rpmRC rc = RPMRC_FAIL;
	char *msg = NULL;
...
static rpmRC process_package(FD_t fdi, FD_t digestori, FD_t validationi)
{
	...
	rpmfi fi = NULL;
	char *msg = NULL;
	std::set<struct digestoffset> offsets;
	uint32_t diglen;
	std::set<std::vector<unsigned char>> ds;
	...
	if (isInDenyList(l.name)) {
		rpmlog(RPMLOG_WARNING,
		       _("package %s is in deny list: conversion skipped\n"),
		       l.name);
		...
		/* this is encoded in the file format, so needs to be fixed size (for now?) */
		...
		zeros = new char[fundamental_block_size] ();
		...
		while (next >= 0) {
			...
			const unsigned char *digestb = rpmfiFDigest(fi, NULL, NULL);
			std::vector<unsigned char>digest(digestb, digestb + diglen);
			auto it = ds.find(digest);
			if (it != ds.end())
			...
			pos += pad;
			ds.insert(digest);
			offsets.insert(digestoffset{digest, pos});
			size = rpmfiFSize(fi);
			rc = (rpmRC) rpmfiArchiveReadToFile(fi, fdo, 0);
			if (rc != RPMRC_OK) {
				char *errstr = rpmfileStrerror(rc);
				rpmlog(RPMLOG_ERR,
				       _("rpmfiArchiveReadToFile failed while extracting \"%s\" with RC %d: %s\n"),
		...
		if (validation_len == -1) {
			rpmlog(RPMLOG_ERR,
			       _("validation output ufdCopy failed\n"));
			rc = RPMRC_FAIL;
			goto exit;
		}
		...
		uint32_t offset_ix = offsets.size();
		if (Fwrite(&offset_ix, sizeof(offset_ix), 1, fdo) != sizeof(offset_ix)) {
			rpmlog(RPMLOG_ERR, _("Unable to write length of table\n"));
			rc = RPMRC_FAIL;
			goto exit;
		}
		if (Fwrite(&diglen, sizeof(diglen), 1, fdo) != sizeof(diglen)) {
			rpmlog(RPMLOG_ERR, _("Unable to write length of digest\n"));
			rc = RPMRC_FAIL;
			goto exit;
		}
		for (auto& offset : offsets) {
			if (Fwrite(offset.digest.data(), diglen, 1, fdo) != diglen) {
				rpmlog(RPMLOG_ERR, _("Unable to write digest\n"));
				rc = RPMRC_FAIL;
				goto exit;
			}
			if (Fwrite(&offset.pos, sizeof(rpm_loff_t), 1, fdo) != sizeof(rpm_loff_t)) {
				rpmlog(RPMLOG_ERR, _("Unable to write offset\n"));
				rc = RPMRC_FAIL;
				goto exit;
			}
		}
		digest_pos = digest_table_pos + sizeof(offset_ix) + sizeof(diglen) + offset_ix * (diglen + sizeof(rpm_loff_t));
		 ...
		 * efficient if it is.
		 */
		pad = pad_to(validation_pos + validation_len + 2 * sizeof(rpm_loff_t) + sizeof(uint64_t),
		...
		struct extents_footer_t footer = {
			.offsets = {
				(off64_t) validation_pos,
				(off64_t) digest_table_pos,
				(off64_t) digest_pos,
			},
			.magic = EXTENTS_MAGIC
		};
		if (Fwrite(&footer, sizeof(footer), 1, fdo) != sizeof(footer)) {
...
int main(int argc, char *argv[])
{
	...
	rsetprogname(argv[0]);	/* Portability call -- see system.h */
