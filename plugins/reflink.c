...
struct reflink_state_s {
	reflink_state_s():fundamental_block_size(0), buffer(NULL), keys(0),
	    keysize(0), table(NULL), fd(0), files(NULL), transcoded(0) {
	}
 ...
 */
inline void *bsearch_r(const void *__key, const void *__base, size_t __nmemb,
		       size_t __size, __compar_d_fn_t __compar, void *__arg)
{
...
static rpmRC reflink_psm_pre(rpmPlugin plugin, rpmte te)
{
	...
	if (Fseek(state->fd, -(sizeof(magic)), SEEK_END) < 0) {
		...
		if (Fseek(state->fd, current, SEEK_SET) < 0) {
			...
			rpmlog(RPMLOG_ERR,
			       _
			       ("reflink: unable to seek back to original location\n"));
		}
		return RPMRC_FAIL;
	}
	...
	if (Fseek
	    (state->fd, -(sizeof(rpm_loff_t) * 2 + sizeof(extents_magic_t)),
	     SEEK_END) < 0) {
...
static rpmRC reflink_fsm_file_install(rpmPlugin plugin, rpmfi fi,
				      ...
				      rpmFsmOp op)
{
	...
	if (S_ISREG(file_mode) && !(rpmfiFFlags(fi) & RPMFILE_GHOST)) {
		...
		snprintf(fullpath, sizeof(fullpath), "%s/%s", rpmfiDN(fi),
			 path);
		...
		if (it != state->inodeIndexes.end()) {
			...
			if (link(hl_target, fullpath) != 0) {
				rpmlog(RPMLOG_ERR,
				       _
				       ("reflink: Unable to hard link %s -> %s due to %s\n"),
		...
		if (rpmfiFNlink(fi) > 1) {
			...
			state->inodeIndexes.insert( {
						   inode, strdup(fullpath)}
			);
		}
		...
		if (dst == -1) {
			rpmlog(RPMLOG_ERR,
			       _
			       ("reflink: Unable to open %s for writing due to %s, flags = %x\n"),
		...
		if (size > 0) {
			...
			fcr.src_offset =
			    find(rpmfiFDigest(fi, NULL, NULL), state);
			if (fcr.src_offset == NOT_FOUND) {
				close(dst);
				rpmlog(RPMLOG_ERR,
				       _("reflink: digest not found\n"));
				return RPMRC_FAIL;
			}
			rpmlog(RPMLOG_DEBUG,
			       _
			       ("reflink: Reflinking %llu bytes at %llu to %s orig size=%ld, file=%lld\n"),
			       fcr.src_length, fcr.src_offset, path, size,
			       fcr.src_fd);
			rc = ioctl(dst, FICLONERANGE, &fcr);
			if (rc) {
				rpmlog(RPMLOG_WARNING,
				       _
				       ("reflink: falling back to copying bits for %s due to %d, %d = %s\n"),
				       path, rc, errno, strerror(errno));
				if (Fseek(state->fd, fcr.src_offset, SEEK_SET) <
				    0) {
					close(dst);
					rpmlog(RPMLOG_ERR,
					       _
					       ("reflink: unable to seek on copying bits\n"));
					return RPMRC_FAIL;
				}
				...
				while (left) {
					len =
					    (left >
					     BUFFER_SIZE ? BUFFER_SIZE : left);
					read =
					    Fread(state->buffer, len, 1,
						  state->fd);
					if (read != len) {
						close(dst);
						rpmlog(RPMLOG_ERR,
						       _
						       ("reflink: short read on copying bits\n"));
						return RPMRC_FAIL;
					}
					written =
					    write(dst, state->buffer, len);
					if (read != written) {
						close(dst);
						rpmlog(RPMLOG_ERR,
						       _
						       ("reflink: short write on copying bits\n"));
						return RPMRC_FAIL;
					}
					left -= len;
				}
				...
				if (rc) {
					rpmlog(RPMLOG_ERR,
					       _
					       ("reflink: Unable to truncate %s to %ld due to %s\n"),
