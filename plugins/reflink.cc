...
struct hardlink_target {
	int dirfd;
	std::string path;
};

using inodeIndexHash = std::unordered_map <rpm_ino_t, hardlink_target>;
...
struct reflink_state_s {
	reflink_state_s():keys(0), keysize(0), fd(0), files(NULL), transcoded(0) {
	}
...
static rpmRC reflink_psm_pre(rpmPlugin plugin, rpmte te)
{
	...
	ssize_t len = sizeof(extents_footer);
	...
	if (state->keys == 0 || state->keysize == 0) {
		...
		for (unsigned int i = 0; i < state->keys; i++) {
...
static rpmRC reflink_psm_post(rpmPlugin plugin, rpmte te, int res)
{
	...
	state->table.clear();
	for (auto &entry : state->inodeIndexes)
		close(entry.second.dirfd);
                                      ...
                                      const char *path, mode_t file_mode, rpmFsmOp op)
{
	...
	if (S_ISREG(file_mode) && !(rpmfiFFlags(fi) & RPMFILE_GHOST)) {
		...
		if (it != state->inodeIndexes.end()) {
			...
			if (linkat(it->second.dirfd, it->second.path.c_str(), dirfd, path, 0) != 0) {
				rpmlog(RPMLOG_ERR,
				       _("reflink: Unable to hard link %s -> %s due to %s\n"),
				       it->second.path.c_str(), path, strerror(errno));
				return RPMRC_FAIL;
			}
		...
		if (rpmfiFNlink(fi) > 1)
			...
			state->inodeIndexes.insert({inode, {dup(dirfd), std::string(path)}});
